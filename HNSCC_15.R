...
#CD3G<-subset(HNSCC,idents = c("0","2","3","5","7","9","10","12","14","15"))
...
#Idents(CD3G)<-CD3G@meta.data$group
...
for (i in 1:26) {
  ...
  #assign(paste("CD8T_",i,"_TIL",sep = ""),subset(CD8T,idents = paste("TIL",i,sep = "")))
  #assign("e",slot(get(paste("CD8T_",i,"_TIL",sep = "")),"meta.data"))
  assign(paste("CD8T_",i,"_TIL",sep = ""),subset(CD8T,idents = paste("TIL",i,sep = "")))
  assign("d",slot(get(paste("CD8T_",i,"_TIL",sep = "")),"meta.data"))
  #assign(paste("Treg_",i,"_TIL",sep = ""),subset(Treg,idents = paste("TIL",i,sep = "")))
...
colnames(scaldata)<-c("JMJD1C","Effective_CD8")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=Effective_CD8)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
