...
for (i in c("wilcox","bimod","roc","t","negbinom","poisson","LR","DESeq2")) {
  if(i == "negbinom" | i == "poisson" | i == "DESeq2"){
    Tregmarker<-FindMarkers(Treg,ident.1 = "TIL",ident.2 = "PBMC",logfc.threshold = 0,min.pct = 0,test.use = i,slot = "counts")
  }else{
    Tregmarker<-FindMarkers(Treg,ident.1 = "TIL",ident.2 = "PBMC",logfc.threshold = 0,min.pct = 0,test.use = i)
  }
  Newname<-intersect(toupper(rownames(epimarker)),rownames(Tregmarker))
  Tregmarker_new<-Tregmarker[Newname,]
  write.table(Tregmarker_new,file = paste0("c:/Users/xjmik/Desktop/HNSCCmarker",i,".txt"),sep = "\t")
  remove(Tregmarker,Tregmarker_new,Newname)
}
