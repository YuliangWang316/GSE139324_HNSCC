...
for (i in 1:26) {
  ...
  assign("c",select(b,one_of("JMJD1C","PDCD1")))
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$JMJD1C),sum(c$PDCD1)))
}
...
colnames(scaldata)<-c("JMJD1C","PDCD1")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=PDCD1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
