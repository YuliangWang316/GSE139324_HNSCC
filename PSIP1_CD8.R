...
VlnPlot(HNSCC,features = c("CD3G","CD8A","CD8B"),pt.size = 0,sort = TRUE)
CD8<-subset(HNSCC,idents = c("5","7","3","10","15"))
Idents(CD8)<-CD8@meta.data$group
Idents(HNSCC)<-HNSCC@meta.data$group
for (i in 1:26) {
  assign(paste("CD8_",i,"_TIL",sep = ""),subset(CD8,idents = paste("TIL",i,sep = "")))
  assign("a",slot(get(paste("CD8_",i,"_TIL",sep = "")),"assays"))
  assign("b",as.data.frame(t(as.data.frame(a$RNA@scale.data))))
  assign("c",select(b,one_of("PSIP1")))
  #assign(paste("CD8T_",i,"_TIL",sep = ""),subset(CD8T,idents = paste("TIL",i,sep = "")))
  #assign("d",slot(get(paste("CD8_",i,"_TIL",sep = "")),"meta.data"))
  #assign(paste("CD3G_",i,"_TIL",sep = ""),subset(CD3G,idents = paste("TIL",i,sep = "")))
  #assign("d",slot(get(paste("CD3G_",i,"_TIL",sep = "")),"meta.data"))
  assign(paste("CD8_",i,"_TIL",sep = ""),subset(CD8,idents = paste("TIL",i,sep = "")))
  assign("e",slot(get(paste("CD8_",i,"_TIL",sep = "")),"meta.data"))
  ...
  f<-(length(rownames(e)))/(length(rownames(d)))
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$PSIP1),f))
...
colnames(scaldata)<-c("PSIP1","CD8T")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=PSIP1,y=CD8T)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
