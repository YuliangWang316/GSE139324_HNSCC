...
remove(HNSCC.data,HNSCC.metadata,all.genes)
...
VlnPlot(HNSCC,features = "CD3G",pt.size = 0,sort = TRUE)
CD8T<-subset(HNSCC,idents = c("5"))

Treg[["percent.mt"]] <- PercentageFeatureSet(Treg, pattern = "^MT-")
VlnPlot(Treg, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3,split.by = "group")
Treg <- subset(Treg, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mt < 10)
...
Treg <- FindNeighbors(Treg, dims = 1:30)
Treg <- FindClusters(Treg, resolution = 0.4)
Treg <- RunUMAP(Treg, dims = 1:30)
Treg <- RunTSNE(Treg, dims = 1:30,check_duplicates = FALSE)
DimPlot(Treg, reduction = "umap",label = TRUE)
DimPlot(Treg, reduction = "umap",split.by = "group")
DimPlot(Treg, reduction = "tsne")
DimPlot(Treg, reduction = "tsne",split.by = "group")
Treg.markers<-FindAllMarkers(Treg,only.pos = TRUE,logfc.threshold = 0.25,min.pct = 0.1)
write.table(Treg.markers,file = "D:/Jmjd1c_Treg_Tumor/GSE139324_code/Treg.markers.txt",sep = "\t")

CD8T[["percent.mt"]] <- PercentageFeatureSet(CD8T, pattern = "^MT-")
VlnPlot(CD8T, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3,split.by = "group")
CD8T <- subset(CD8T, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mt < 10)
CD8T <- NormalizeData(CD8T)
CD8T <- FindVariableFeatures(CD8T, selection.method = "vst", nfeatures = 2000)
all.genes <- rownames(CD8T)
CD8T <- ScaleData(CD8T, features = all.genes)
...
CD8T <- RunTSNE(CD8T, dims = 1:20,check_duplicates = FALSE)
DimPlot(CD8T, reduction = "umap",label = TRUE)
DimPlot(CD8T, reduction = "umap",split.by = "group")
DimPlot(CD8T, reduction = "tsne")
DimPlot(CD8T, reduction = "tsne",split.by = "group")
CD8T.markers<-FindAllMarkers(CD8T,only.pos = TRUE,logfc.threshold = 0.25,min.pct = 0.1)
write.table(CD8T.markers,file = "D:/Jmjd1c_Treg_Tumor/GSE139324_code/CD8T.markers.txt",sep = "\t")