...
for (i in 1:26) {
  assign(paste("HNSCC_",i,"_PBMC.data",sep = ""),Read10X(paste("D:/Jmjd1c_Treg_Tumor/GSE139324_MEATA/HNSCC_",i,"_PBMC",sep = "")))
  ...
  assign(paste("HNSCC_",i,"_TIL.data",sep = ""),Read10X(paste("D:/Jmjd1c_Treg_Tumor/GSE139324_MEATA/HNSCC_",i,"_TIL",sep = "")))
...
VlnPlot(Treg,features = c("JMJD1C","NRP1","PDCD1","IFNG"),split.by = "group")
FeaturePlot(Treg,features =  c("JMJD1C","NRP1","PDCD1","IFNG"),split.by = "group")
scaldata<-as.data.frame(Treg@assays[["RNA"]]@scale.data)
scaldata<-as.data.frame(t(scaldata))
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=NRP1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
ggplot(data = scaldata,aes(x=JMJD1C,y=PDCD1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
ggplot(data = scaldata,aes(x=JMJD1C,y=IFNG)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 

library(dplyr)
scaldata_new<-filter(scaldata,NRP1 >0 , JMJD1C >0)
ggplot(data = scaldata_new,aes(x=JMJD1C,y=NRP1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata_new,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
scaldata_new2<-filter(scaldata,JMJD1C >0,PDCD1>0)
ggplot(data = scaldata_new2,aes(x=JMJD1C,y=PDCD1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata_new2,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
scaldata_new3<-filter(scaldata,JMJD1C >0,IFNG>0)
ggplot(data = scaldata_new3,aes(x=JMJD1C,y=IFNG)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata_new3,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 

K<-Idents(HNSCC)
remove(scaldata,scaldata_new,scaldata_new2)
Idents(Treg)<-Treg@meta.data$group
Treg_PBMC<-subset(Treg,idents = "PBMC")
Treg_TIL<-subset(Treg,idents = "TIL")
remove(HNSCC,Treg)
scaldata<-as.data.frame(Treg_PBMC@assays[["RNA"]]@scale.data)
scaldata<-as.data.frame(t(scaldata))
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=NRP1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
...
library(dplyr)
scaldata_new<-filter(scaldata,JMJD1C >0,NRP1>0)
ggplot(data = scaldata_new,aes(x=JMJD1C,y=NRP1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata_new,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
scaldata_new2<-filter(scaldata,JMJD1C >0,PDCD1>0)
ggplot(data = scaldata_new2,aes(x=JMJD1C,y=PDCD1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata_new2,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 

remove(Treg_PBMC)
scaldata<-as.data.frame(Treg_TIL@assays[["RNA"]]@scale.data)
scaldata<-as.data.frame(t(scaldata))
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=NRP1)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=FALSE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
