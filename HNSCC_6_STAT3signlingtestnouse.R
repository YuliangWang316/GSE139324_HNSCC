...
a_new<-a[2:87,]
b_new<-b[2:104,]
Treg<-AddModuleScore(Treg,features = a_new,name = "STAT3",random.seed = 1)
Treg<-AddModuleScore(Treg,features = b_new,name = "PI3K",random.seed = 1)
...
for (i in 1:26) {
  ...
  assign("e",as.data.frame(d$STAT35))
  colnames(e)<-"STAT3"
...
rm(scaldata_1_TIL,scaldata_10_TIL,scaldata_11_TIL,scaldata_12_TIL,scaldata_13_TIL,scaldata_14_TIL,scaldata_15_TIL,scaldata_16_TIL,scaldata_17_TIL,scaldata_18_TIL,scaldata_19_TIL,scaldata_2_TIL,scaldata_21_TIL,scaldata_22_TIL,scaldata_23_TIL,scaldata_24_TIL,scaldata_25_TIL,scaldata_26_TIL,scaldata_3_TIL,scaldata_4_TIL,scaldata_5_TIL,scaldata_6_TIL,scaldata_7_TIL,scaldata_8_TIL,scaldata_9_TIL,scaldata_20_TIL,a_new,b_new,i)
colnames(scaldata)<-c("JMJD1C","STAT3_signaling")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=JMJD1C,y=STAT3_signaling)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
