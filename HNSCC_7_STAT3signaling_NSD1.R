...
for (i in 1:26) {
  ...
  colnames(e)<-"STAT3"
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(sum(c$NSD1),sum(e$STAT3)))
}
...
colnames(scaldata)<-c("NSD1","STAT3_signaling")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=NSD1,y=STAT3_signaling)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
