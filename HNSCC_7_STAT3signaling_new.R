...
library(patchwork)
library(tidyverse)
#seurat 4.0.6, #seuratobject 4.0.4
...
remove(all.genes,HNSCC)


Idents(Treg)<-Treg@meta.data$group
a<-read.table("D:/Jmjd1c_Treg_Tumor/GSE139324_code/geneset.txt",sep = "\t",header = TRUE)
b<-read.table("D:/Jmjd1c_Treg_Tumor/GSE139324_code/geneset (1).txt",sep = "\t",header = TRUE)
a_new<-a[2:87,]
b_new<-b[2:104,]
#Treg<-AddModuleScore(Treg,features = a_new,name = "STAT3")
#Treg<-AddModuleScore(Treg,features = b_new,name = "PI3K")
stat3<-FetchData(Treg,vars = a_new)
for (i in 1:length(rownames(stat3))) {
  stat3$Average[i]<-mean(as.numeric(stat3[i,]))
}

Total<-FetchData(Treg,vars = rownames(Treg))
Data<-data.frame()
for (j in 1:length(colnames(Total))) {
  Data[1,j]<-mean(Total[,j])
}
average_stat3<-mean(stat3$Average)
...
Sortdatahi<-filter(Sortdata,S > average_stat3)
Sortdatalo<-filter(Sortdata,S < average_stat3)
higene<-rownames(Sortdatahi[(length(rownames(Sortdatahi))-(10*length((a_new)))/2+1):length(rownames(Sortdatahi)),])
logene<-rownames(Sortdatalo[1:(10*length((a_new))/2),])
k<-c(higene,logene)
control<-FetchData(Treg,vars = k )
for (p in 1:length(rownames(control))) {
  control$Average[p]<-mean(as.numeric(control[p,]))
}

Treg@meta.data$Stat3_1<-stat3$Average-control$Average
...
#Treg<-AddModuleScore(Treg,features = a_new,name = "STAT3")
#Treg<-AddModuleScore(Treg,features = b_new,name = "PI3K")
#stat3<-FetchData(HNSCC,vars = a_new)
#for (i in 1:length(rownames(stat3))) {
#  stat3$Average[i]<-mean(as.numeric(stat3[i,]))
#}
#
#Total<-FetchData(HNSCC,vars = rownames(HNSCC))
#Data<-data.frame()
#for (j in 1:length(colnames(Total))) {
#  Data[1,j]<-mean(Total[,j])
#}
...
#logene<-rownames(Sortdatalo[1:(10*length((a_new))/2),])
#k<-c(higene,logene)
#control<-FetchData(HNSCC,vars = k )
#for (p in 1:length(rownames(control))) {
#  control$Average[p]<-mean(as.numeric(control[p,]))
#}
#
#HNSCC@meta.data$Stat3_1<-stat3$Average-control$Average

library(dplyr)
remove(a,b,a_new,b_new,control,Data,Data_T,rownamesDataT,Sortdata,Sortdatahi,Sortdatalo,stat3,Total,average_stat3,higene,i,j,k,logene,p)
...
#  STAT3<-select(d,starts_with("Stat3_1"))
#  d$STAT3_2<-apply(STAT3,MARGIN = 1,median)
#  PI3K<-select(d,starts_with("PI3K"))
#  d$PI3K<-apply(PI3K,MARGIN = 1,median)
  assign("e",as.data.frame(d$Stat3_1))
  colnames(e)<-"STAT3"
